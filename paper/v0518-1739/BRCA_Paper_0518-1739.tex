...
% Version: 0518-1739  (ctexbook + TOC/LOF/LOT + template format + 审稿修正v2)
% ===========================================================================
...
\caption[上调基因富集气泡图]{上调基因富集气泡图（Top 25, FDR$<0.05$）。横轴$-\log_{10}(p)$，纵轴通路名称按显著性降序。气泡大小=交叠基因数量，颜色区分来源数据库（scale\_color\_brewer(palette="Set1")，六数据库）。前五位全部为Reactome细胞周期相关条目。}\label{fig:enrich_up}
\end{figure}
...
XGBoost的准确率仅为35.8\%（Kappa=0.002）。由于数据集存在严重的类别不平衡（Luminal A占86.3\%，即950/1,102），最简单的"全部预测为Luminal A"策略即可达到86.3\%的准确率。XGBoost 35.8\%不仅远低于这一多数类基线（低约51个百分点），甚至与三分类随机猜测（33.3\%）几无差异——任何正确实现的多分类器在训练后可自然退化为预测多数类来保底获得86.3\%准确率，低于此基准意味着模型的实际预测行为已偏离随机期望。此类异常通常指向代码实现层面的问题，而非超参数或模型复杂度层面的不足，建议首先排查：（1）XGBoost的objective参数是否设置为multi:softmax/multi:softprob（而非默认的binary:logistic或reg:squarederror），（2）训练集和测试集的类别标签编码是否因数据划分方式不一致而导致映射错位，（3）预测时是否误将概率矩阵而非类别标签作为最终输出。在排除上述代码层面错误后重新评估，XGBoost的准确率应至少接近或超过86\%的多数类基线。LASSO的89.4\%准确率仅比多数类基线高约3个百分点——这一"边际提升"在三个类别（而非常见的两类）的判别任务中更稳健，但不宜被解读为颠覆性性能优势。
...
\caption[TMB按分子亚型分布]{TMB按分子亚型分布箱线图。箱体=IQR，内部横线=中位数，须线=1.5$\times$IQR。绘图参数：fill=npg10[1:4]（4种亚型）。TN中位TMB（1.47/Mb）为Luminal A（0.79/Mb）的1.86倍。HER2-enriched 37例中35例具备TMB数据。}\label{fig:tmb}
\end{figure}

\section{关联规则与调控网络}

miRNA-mRNA调控相关性热图（图\ref{fig:mirna}）展示了Top 20 miRNA-mRNA配对（按$|r|$排序）。横轴为mRNA的Ensembl ID，纵轴为miRNA标识，每个格子的颜色由Pearson $r$值编码——从蓝色（负相关）经白色（$r\approx0$）渐变至红色（正相关），格子内标注具体$r$值。以$|r|>0.3$为阈值共识别228对显著调控关系对（候选空间：Top 50 miRNA $\times$ Top 50 mRNA = 2,500对），其中57.9\%为负相关、42.1\%为正相关。负相关占优符合miRNA通过碱基配对结合靶mRNA并导致其降解或翻译抑制的经典调控机制。正相关关系并不违背miRNA的经典机制，而可能反映间接调控、反馈回路或共同上游转录因子的影响。值得注意的是，在另一项分析中以更严格的$|r|>0.4$阈值扫描了更大候选空间（100 miRNA $\times$ 2,292 DEGs $\approx$ 229,200对），共识别2,065对，但正相关占绝大多数（87.1\%）——两次分析不可直接比较，因为候选空间扩大了约92倍（2,500 vs 229,200）。更严格阈值却检出更多关系对这一表面反直觉现象，根源在于候选特征池从50$\times$50扩展到100$\times$2,292，而非方法学矛盾。大规模扫描中正相关关系占优这一现象提示，在候选空间充分大的条件下，随机共表达效应可能压倒直接的miRNA-target负调控信号。
...
\caption[miRNA-mRNA调控相关性热图]{miRNA-mRNA调控相关性热图（Top 20, $|r|>0.3$）。行=miRNA, 列=mRNA, 格内标注$r$值。色阶：scale\_fill\_gradient2(low=npg10[5], mid=white, high=npg10[3])。228对中以负相关为主（57.9\%）。}\label{fig:mirna}
\end{figure}
...
\textbf{分类与聚类的一致性}：LASSO以89.4\%准确率仅用35个基因实现三分类预测，验证了亚型间转录组差异的稀疏性。需要指出，鉴于Luminal A占多数（86.3\%），LASSO相较于朴素多数类基线的实际提升约为3个百分点——在严重类别不平衡场景中这一提升的意义比绝对数值更值得关注，但不宜过度解读。XGBoost 35.8\%远低于多数类基线，提示梯度提升类算法在该数据条件下（高维、小样本、极不平衡）面临根本性困难。PCA、t-SNE和K-means一致指向ER状态为最显著的分组驱动因素。PC1仅解释了15.3\%的方差，说明ER虽然是最强单因素，但BRCA转录组的全局结构相当分散——绝大多数变异维度不与已知分类标签对齐，这部分"未解释方差"是分型精细化的主要空间。
