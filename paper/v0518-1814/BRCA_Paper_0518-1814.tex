...
% Version: 0518-1814  (ctexbook + TOC/LOF/LOT + template format + 审稿修正v5)
% ===========================================================================
...
临床数据清洗：从94个原始变量中提取25个核心变量，涵盖基本信息（确诊年龄、性别、种族）、病理特征（AJCC分期、TNM分期、组织学类型）、分子分型指标（ER、PR、HER2免疫组化状态）、治疗信息及生存信息。分子亚型根据ER/PR/HER2免疫组化状态分类。本研究未使用Ki-67增殖指数和PAM50基因表达签名进行亚型判定，因此Luminal A与Luminal B的区分依赖于有限的IHC指标，可能导致Luminal A比例偏高——这是临床IHC分类的固有局限，非数据错误。临床分期方面，将AJCC病理分期中的亚分期字母去除，统一为Stage I至IV四个类别。
...
\section{数据概览}
经预处理和跨组学整合，最终分析数据集概况见表\ref{tab:data}。mRNA覆盖1,105例肿瘤（另113例正常），miRNA覆盖1,079例。以mRNA肿瘤样本为基准，多组学对齐后获得1,076例共同患者（mRNA—miRNA重合率97.3\%），突变数据覆盖990例。临床特征中Luminal A占绝对优势（950例，86.3\%），HER2-enriched仅37例（3.4\%），Triple Negative 115例（10.4\%）；Stage II占比最高（58.9\%），Stage IV仅20例（1.8\%）。中位OS随访约2.7年，死亡事件87例（7.9\%），事件率偏低是后续预后模型构建面临的核心约束。
...
\caption[PCA主成分分析]{PCA主成分分析（Top 2,000可变基因, 95\%置信椭圆）。绘图参数：Luminal A=npg10[1], Luminal B=npg10[2], HER2-enriched=npg10[3], Triple Negative=npg10[4]。注意Luminal B仅3例在图中几乎不可见——分类模型已将其过滤（仅使用3类）。PC1方向按ER状态分离；LA簇最紧凑，TN离散度最大，HER2居中。}\label{fig:pca}
\end{figure}
...
\textbf{局限性}：（1）DNA甲基化数据（11.7 GB）未纳入；（2）生存事件率较低（7.9\%）致Cox模型对Stage IV HR的估计不稳定（CI宽度较大）；（3）miRNA-mRNA调控基于相关而非因果，且两次分析的候选空间和结论方向相反，提示结果高度依赖于阈值和特征集选择；（4）WGCNA grey模块占比过高（56.3\%），提示软阈值power=8和minModuleSize=30的参数组合可能未达最优——降低minModuleSize或调整mergeCutHeight可能改善基因分配效率，需在后续分析中优化；（5）共识聚类C4--C8（59例）缺乏独立验证证据支撑其生物学意义。
