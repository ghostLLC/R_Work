...
theme_big <- theme_classic(base_size = 15) +
  theme(
...
p1 <- ggplot(vdf, aes(lfc, lp)) +
  ...
  annotate("text", x = 4.5, y = 3, label = sprintf("Up: %d", sum(vdf$sig == "Up")),
           color = npg10[3], size = 5, fontface = "bold") +
  annotate("text", x = -4.5, y = 3, label = sprintf("Down: %d", sum(vdf$sig == "Down")),
           color = npg10[5], size = 5, fontface = "bold") +
  theme_big_nolegend
...
th7 <- theme_classic(base_size = 17) +
  theme(
...
p7a <- ggplot(sft, aes(Power, SFT_R2)) +
  ...
  annotate("text", x = 13, y = 0.83, label = expression(R^2 == 0.887), size = 5, parse = TRUE) +
...
p8 <- ggplot(ms, aes(x = reorder(Module, Size), y = Size, fill = Module)) +
  ...
  geom_text(aes(label = Size), hjust = -0.3, size = 8, fontface = "bold") +
