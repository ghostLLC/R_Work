...
% Version: 0518-1805  (ctexbook + TOC/LOF/LOT + template format + 审稿修正v4)
% ===========================================================================
...
表达数据归一化：原始Read Counts受基因长度和测序深度双重因素影响，不适合直接用于跨基因比较。本研究采用TPM（Transcripts Per Million）作为归一化指标，计算分两步：将每个基因的Read Counts除以该样本的总Read Counts再乘以$10^6$以校正测序深度（CPM），再将CPM除以对应基因的有效长度（千碱基）以校正基因长度。基因长度信息通过Ensembl基因组注释数据库（GRCh38/hg38）获取。需注意DESeq2差异表达分析使用原始计数（Raw Counts）作为输入以符合其统计假设，TPM主要用于聚类、WGCNA和可视化等下游分析。
...
\section{分子亚型分类}
三种分类器在独立测试集上的性能呈现明显的层次分化（图\ref{fig:model}，表\ref{tab:class}）：LASSO最优，RF和XGBoost接近（准确率相同均为86.7\%，Kappa仅差0.001）。LASSO以89.4\%准确率排序第一（Kappa=0.528），仅使用从500个候选基因中筛选出的35个区分性基因——压缩比约为14:1。这一高压缩比揭示了BRCA分子亚型之间存在较强的稀疏判别结构：真正决定亚型分类的关键表达特征并不需要大量基因，而集中于少数具有高度判别能力的核心标志物。随机森林准确率为86.7\%（Kappa=0.365），其Macro F1分数（0.706）在三个模型中最高，高于LASSO（0.612）和XGBoost（0.518）——这是因为F1 Macro对每个类别等权重计算，随机森林在少数类（HER2-enriched仅37例、Triple Negative仅115例）上保持了更好的召回率。在类别不平衡的医学分类任务中，单纯追求Accuracy往往容易偏向多数类，而Macro F1更能反映模型对所有亚型的综合识别能力。
...
共识聚类将1,105例肿瘤样本分为8个共识簇，其中前三个主要簇（C1: 474例, 42.9\%; C2: 433例, 39.2\%; C3: 139例, 12.6\%）合计覆盖了94.6\%的样本（表\ref{tab:consensus}）。需要说明：K=8在本文中为算法评估上限（代码中设置maxK=8），而非通过delta area曲线或PAC指标选定的最优K——这是当前分析的一个方法学局限。C1和C2构成样本主体的二分结构（合计82.1\%）与K-means最优$K=2$及PCA的ER+/ER-分离趋势一致，表明二分结构是数据的首要分组特征；C3及C4--C8则代表对二分主干的进一步细化。簇特异性标记基因通过簇内高表达和簇间差异筛选——Top标记基因（如ENSG00000164611, $p=2.5\times10^{-29}$）代表了定义各共识簇分子特征的核心转录信号。
...
TMB按分子亚型分层（图\ref{fig:tmb}）揭示了一个具有临床意义的差异：Triple Negative的中位TMB为1.47/Mb，是Luminal A（0.79/Mb）的1.86倍。HER2-enriched的中位TMB（1.66/Mb）在这三个亚型中最高，但HER2-enriched具备TMB数据的样本仅35例（最小的亚型组），因此中位数的估计精度受限。Luminal B因仅有约3例具备TMB数据，未纳入比较分析——如此小的样本量无法提供有意义的中位数估计。Luminal A的分布最为集中，其TMB均值（1.67）远高于中位数（0.79），说明存在少量高TMB离群样本拉高了均值。TNBC更高TMB的免疫学意义在于：更高的突变数量理论上能够产生更多新抗原，从而提高肿瘤被免疫系统识别的概率——这为TNBC对免疫检查点抑制剂相对更敏感提供了基因组层面的潜在机制解释。但需注意BRCA的TMB绝对值远低于黑色素瘤和肺癌等典型高TMB癌种，其作为免疫治疗预测标志物的临床价值需要结合具体阈值评估。
...
miRNA-mRNA调控相关性热图（图\ref{fig:mirna}）展示了Top 20 miRNA-mRNA配对（按$|r|$排序）。该分析采用聚焦候选集策略：Top 50最可变miRNA $\times$ Top 50最可变mRNA，候选空间为2,500对。横轴为mRNA的Ensembl ID，纵轴为miRNA标识，每个格子的颜色由Pearson $r$值编码——从蓝色（负相关）经白色（$r\approx0$）渐变至红色（正相关），格子内标注具体$r$值。以$|r|>0.3$为阈值共识别228对调控关系对，其中57.9\%为负相关、42.1\%为正相关。负相关占优符合miRNA通过碱基配对结合靶mRNA并导致其降解或翻译抑制的经典调控机制。正相关关系可能反映间接调控、反馈回路或共同上游转录因子的影响。
...
K-means轮廓系数在$K=2$时最大（0.18），提示数据最自然的分组为ER+/ER-二分结构。然而共识聚类经过1,000次bootstrap重采样识别出8个聚类（C1--C8），其中C1--C3涵盖94.6\%的样本（合计1,046/1,105例），而C4--C8合计仅59例（5.3\%）。两种方法给出不同的最适$K$值反映了两者评估框架的根本差异：K-means的轮廓系数假设球形聚类结构且在无外部验证的情况下更偏好低$K$，而共识聚类通过重采样的聚类一致性来判定分组稳定性，对小规模但高度可重现的子群更为敏感。当前C4--C8是否为具有生物学意义的新亚型，需要独立队列验证和差异表达特征的功能富集证据支持——仅凭聚类一致性指标不足以断言这些为真实生物学子群。PCA和t-SNE一致指向ER状态为最显著的分组驱动因素。PC1仅解释了15.3\%的方差，说明ER虽然是最强单因素，但BRCA转录组的全局结构相当分散——绝大多数变异维度不与已知分类标签对齐，这部分"未解释方差"是分型精细化的主要空间。
...
\textbf{突变与TMB}：TP53（34.4\%）和PIK3CA（33.9\%）的高频突变及互斥趋势与已知BRCA驱动突变谱一致。TMB在TNBC中显著高于Luminal A（1.86倍），这一差异为TNBC对免疫治疗应答率较高的临床观察提供了基因组层面的潜在解释——但需注意BRCA的TMB绝对值整体偏低，不宜与其他高TMB癌种直接类比。
