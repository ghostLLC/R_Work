...
% Version: 0518-1753  (ctexbook + TOC/LOF/LOT + template format + 审稿修正v3)
% ===========================================================================
...
\caption[分类模型性能对比]{分类模型性能对比分组柱状图。每组三根柱分别对应Accuracy、Kappa和F1 Macro。绘图参数：fill=npg10[1:3]（3种模型）。LASSO在Accuracy和Kappa上最优，RF在F1 Macro上最优。}\label{fig:model}
\end{figure}
...
共识聚类将1,105例肿瘤样本分为8个共识簇，其中前三个主要簇（C1: 474例, 42.9\%; C2: 433例, 39.2\%; C3: 139例, 12.6\%）合计覆盖了94.6\%的样本（表\ref{tab:consensus}）。C1和C2构成样本主体的二分结构与K-means最优$K=2$及PCA的ER+/ER-分离趋势一致，而C3及后续小簇可能对应具有特殊分子特征的样本子群。簇特异性标记基因通过簇内高表达和簇间差异筛选——Top标记基因（如ENSG00000164611, $p=2.5\times10^{-29}$）代表了定义各共识簇分子特征的核心转录信号。
