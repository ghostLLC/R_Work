...
doc <- TXT(doc, "采用Apriori算法（arules包）对基因表达高/低状态与临床特征之间的关联规则进行挖掘。图3.12以散点图展示关联规则的全局分布：横轴为Support，纵轴为Confidence，点的大小和颜色编码Lift值（>1表示正关联）。图中位于右上角区域的规则兼具高Support和高Confidence——这些规则对应的基因-临床特征关联在数据中既普遍又可靠。")
