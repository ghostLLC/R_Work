...
FIG(doc, file.path(FIG_DIR, "fig1_volcano.png"),
    "图3.1 差异表达火山图（Tumor vs Normal, 6,768 DEGs, |log2FC|>1/FDR<0.05, NPG色板）", 4.8, 3.6)
...
FIG(doc, file.path(FIG_DIR, "fig12_enrichment_up.png"),
    "图3.2 上调基因GO/KEGG/Reactome富集气泡图（Top 25显著条目, FDR<0.05）", 4.8, 3.0)
...
FIG(doc, file.path(FIG_DIR, "fig13_enrichment_down.png"),
    "图3.3 下调基因富集气泡图（Top 25显著条目, 集中于ECM组织/黏附通路）", 4.8, 3.0)
...
FIG(doc, file.path(FIG_DIR, "fig16_enrichment_top10.png"),
    "图3.4 全部DEGs Top 10最显著富集通路（横轴为-log10(p), 来源数据库分色）", 4.8, 2.4)
...
FIG(doc, file.path(FIG_DIR, "fig6_model_comparison.png"),
    "图3.5 分子亚型分类模型性能对比（LASSO/RF/XGBoost, 三指标分组柱状图）", 4.5, 3.5)
...
FIG(doc, file.path(FIG_DIR, "fig2_pca_subtype.png"),
    "图3.6 PCA主成分分析（Top 2,000可变基因, PC1 15.3% × PC2 8.5%, NPG四色分子亚型, 95%置信椭圆）", 4.8, 3.6)
...
FIG(doc, file.path(FIG_DIR, "fig8_wgcna_modules.png"),
    "图3.8 WGCNA共表达模块大小分布（8个有效模块, excl. grey, Soft Power=8）", 3.5, 2.6)
...
FIG(doc, file.path(FIG_DIR, "fig3_km_stage.png"),
    "图3.9 Kaplan-Meier生存曲线（Stage I-IV分层, log-rank p<0.0001, 含Number at Risk表）", 4.5, 4.0)
...
FIG(doc, file.path(FIG_DIR, "fig9_cox_forest.png"),
    "图3.10 多变量Cox回归森林图（HR及95%CI, Stage IV HR=8.74, C-index=0.771）", 4.5, 3.5)
...
FIG(doc, file.path(FIG_DIR, "fig4_oncoplot.png"),
    "图3.11 体细胞突变瀑布图（Top 12突变基因, 990例样本, PIK3CA 37.3%/TP53 35.2%）", 5, 2.5)
...
doc <- TXT(doc, "采用Apriori算法（arules包）对基因表达高/低状态与临床特征（分子亚型、ER/PR/HER2状态）之间的关联规则进行挖掘。图3.12以散点图展示关联规则的全局分布：横轴为Support（规则在全部样本中的覆盖比例），纵轴为Confidence（规则前提成立时结论也成立的概率），点的大小和颜色均编码Lift值（规则提升度，>1表示正关联）。")
...
FIG(doc, file.path(FIG_DIR, "fig15_association_rules.png"),
    "图3.12 关联规则散点图（Support × Confidence, Lift着色, Apriori算法）", 4.5, 3.1)
FIG(doc, file.path(FIG_DIR, "fig10_mirna_mrna_corr.png"),
    "图3.13 miRNA-mRNA调控相关性热图（Top 20配对, |r|>0.4, 红-白-蓝色阶）", 4.5, 3.4)
...
FIG(doc, file.path(FIG_DIR, "fig5_deg_heatmap.png"),
    "图4.1 Top 25 DEGs表达热图（Tumor vs Normal, Z-score标准化）", 4.8, 3.6)
...
FIG(doc, file.path(FIG_DIR, "fig11_summary.png"),
    "图4.4 综合分析六面板汇总图（火山图/亚型分布/模型对比/KM/分期/WGCNA）", 5, 3.3)
